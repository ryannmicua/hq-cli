...
Start with `docs/plans/2026-07-24-phase-0-contracts-and-reads.md`. Before the first code edit, set that phase and its first task to `in-progress` in `implementation/STATUS.md`. If commits are not explicitly authorized, skip commit steps and retain the verified working tree for review.
